$().ready(function() {
    ...
    function selectFile(f, onProgress, onComplete) {
        (function() {
            ...
            progressiveRead(f,
                     ...
                     //   $(file.previewElement).find('.dz-progress .dz-upload').css('width', progress + '%');
                        ...
                        if (onProgress) {
                            onProgress({
                                change: progress,
                                filename: file.name,
                                currentsize: bytes2si2(pos, file.size, 2),
                                totalsize: bytes2si(pos / took, 2),
                                totaltime: took
                            });
                        }
                ...
                function(file) {
                 //   $(file.previewElement).removeClass('dz-progressing');
                    ...
                    if (onComplete) {
                        onComplete({ hash : hash, filename: file.name});
                    }
    ...
    window.initDropZone = function (el, callbacks) {

        if (!compatible()) {
            alert('HashProof requires HTML5 browser');
            return;
        }
        ...
        Dropzone.options[el] = {
            ...
            init: function() {
                ...
                if (callbacks.onInitDrop) {
                    callbacks.onInitDrop(this);
                }
                ...
                this.on("addedfile",
                    function(file) {
                        if (callbacks.onFileDrop) {
                            callbacks.onFileDrop({filename:file.name, filesize:file.size, dropel:this});
                        }
                        selectFile(file, callbacks.onProgress, callbacks.onComplete);
